...
<div class="card" style="max-width:600px;">
    ...
    <form action="{{ route('admin.rooms.store') }}" method="POST" enctype="multipart/form-data" novalidate>
        @csrf
        ...
        <div class="field">
            ...
            <select name="room_category_id" @class(['is-invalid' => $errors->has('room_category_id')])>
                ...
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}" @selected(old('room_category_id') == $category->id)>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
